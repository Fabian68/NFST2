...
void David::attaqueEnnemis(sf::RenderWindow* window)
{
	...
	case 3:
		...
		for (int i = 0;i < equipeEnnemi().taille();i++) {
			if (equipeEnnemi()[i]->estEnVie()) {
				...
				Attaque(DEGATS+vitesse()/5, equipeEnnemi()[i],window);	
				if (habile()) {
					DEGATS = static_cast<int>(Aleatoire(j, j * 2).decimal() * (vitesse() * 1.0));
...
void David::passif(int tour, sf::RenderWindow* window)
{
	if (vie() == vieMax()) {
...
void David::passifDefensif(sf::RenderWindow* window)
{
}
