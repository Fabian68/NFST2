...
void Personnage::soigner(int soins, Combat& C, Personnage* P, sf::RenderWindow* window)
{
	if (P->estEnVie()) {
		...
		if (possedeObjetNumero(OBJET_SCEPTRE_DRUIDE) && Aleatoire(0, 101).entier() < 20) {
...
void Personnage::bouclier(int soins, Combat& C, Personnage* P, sf::RenderWindow* window)
{
	if (P->estEnVie()) {
		...
		if (possedeObjetNumero(OBJET_SCEPTRE_DRUIDE) && Aleatoire(0, 101).entier() < 10) {
...
void Personnage::traitementAnimaux(Combat& C, sf::RenderWindow* window, std::vector< sf::Sound >& allSounds) {
	if (_animal.indice() > 0 && _animal.activer(_rareteAnimal))
	{
...
int Personnage::calcul_ajout_degats(int Degat, Personnage* Defenseur) {
	...
	if (Aleatoire().entier() <= _chanceHabilete && possedeObjetNumero(OBJET_CAPE_HABILETE)) {
	...
	if (possedeObjetNumero(OBJET_CASSE_NOIX) && Defenseur->possedeBouclier()) {
...
void  Personnage::Attaque(int Degat, Personnage* Defenseur, Combat& C, sf::RenderWindow* window, std::vector< sf::Sound >& allSounds)
{
	...
	if (Defenseur->estAttaquable()) {
		if (Defenseur->id() == ID_EVAN && Defenseur->id() != Defenseur->equipeAllier().plusProcheVivant()->id()) {
			...
			if (Defenseur->possedeObjetNumero(OBJET_MARHSMALLOW) && Aleatoire(0, 101).entier() < 20) {
				Defenseur->bouclier(Degat, C, Defenseur, window);
			}
			...
			if (Defenseur->devie()) {
				...
				if (!redirection) {
					Degat = calcul_ajout_degats(Degat, Defenseur);
					Degat = calcul_reduction_degats(Degat, Defenseur);
		...
		if (Defenseur->possedeObjetNumero(OBJET_DAGUE_NINJA)) {
			...
			deg = (int)((deg*1.0) * (_degatCritique / 100.0 + 1.0));
